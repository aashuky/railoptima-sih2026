 ...
 * 
 * Rescaled Factor Weights (Genuinely sum to exactly 100):
 * 1. Corridor Match (Hard Requirement): 23 pts. Tasks MUST share the exact corridor segment. Mismatched => 0 pts.
 * 2. Window Capacity Fit: 23 pts. Combined task duration must fit within the block window.
 * 3. Cross-Department Synergy: 30 pts. Multi-department combinations maximize asset availability:
 *    - 3 Departments (TMS + SMMS + TDMS): 30 pts (Full synergy bonus)
 *    - 2 Departments: 20 pts
 *    - 1 Department (Single): 8 pts
 * 4. Non-Conflicting Work Types & Safety: 12 pts. Parallel track, signal, and OHE safety clearances.
 * 5. Priority & Urgency Weight: 12 pts. Urgency alignment for High-priority critical infrastructure items:
 *    - >= 2 High-priority tasks: 12 pts
 *    - 1 High-priority task: 9 pts
 *    - Medium/Low priority tasks: 6 pts
 * 
 * Total: 23 + 23 + 30 + 12 + 12 = 100 points maximum.
 */
...
function calculateCompatibility(tasks, block) {
  ...
  if (totalDuration <= maxAllowedDuration) {
    ...
    // At 100% utilization: 15 + 8 = 23 pts. At 75%: 15 + 6 = 21 pts.
  ...
  let departmentSynergy = 0;
  if (departments.size >= 3) {
    departmentSynergy = 30; // Full Tri-department synergy (TMS + SMMS + TDMS)
    reasons.push(
    ...
    reasons.push(
      `Bi-department synergy achieved: Merges ${Array.from(departments).join(" & ")} into a single window, eliminating 1 train disruption.`
    );
  } else {
    departmentSynergy = 8;
  ...
  let safetyFeasibility = 12;
  const hasTrack = departments.has("TMS");
  const hasSignal = departments.has("SMMS");
  const hasTraction = departments.has("TDMS");
  ...
  // Compute final score - exact sum of factors (23 + 23 + 30 + 12 + 12 = 100)
